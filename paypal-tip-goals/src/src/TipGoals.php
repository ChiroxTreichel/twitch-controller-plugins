...
final class TipGoals
{
     ...
     */
    public static function brand(App $app): string
    {
        $eigener = trim($app->settings->string('brand', '', self::scope()));

        if ($eigener !== '') {
            return $eigener;
        }

        $kanal = $app->settings->string('twitch_broadcaster_name');

        return $kanal !== '' ? $kanal : $app->settings->string('twitch_broadcaster_login');
    }

    /** Der kleinste Betrag, den die Seite annimmt. */
    public static function minAmount(App $app): float
    {
        return max(0.5, (float) $app->settings->string('min_amount', '1', self::scope()));
    }

    /** Der Betrag, der im Feld vorsteht. */
    public static function defaultAmount(App $app): float
    {
        return max(self::minAmount($app), (float) $app->settings->string('default_amount', '5', self::scope()));
    }

    /**
     * Die Knoepfe mit festen Betraegen.
     *
     * @return list<float>
     */
    public static function presets(App $app): array
    {
        $roh = trim($app->settings->string('presets', '1, 2, 5, 10', self::scope()));

        $werte = [];
        foreach (preg_split('/[,;\s]+/', $roh) ?: [] as $eines) {
            $zahl = (float) self::money($eines);

            if ($zahl > 0 && !in_array($zahl, $werte, true)) {
                $werte[] = $zahl;
            }
        }
    ...
    /**
     * Einen Betrag auf ein Ziel schreiben.
     *
     * Hier liegt der Unterschied zu den Tip-Goals von StreamElements
     * und Streamlabs: dort kommt eine Spende aus einer Schnittstelle
     * und weiss nichts von Zielen, also landet sie auf dem obersten.
     * Hier hat der SPENDER gewaehlt, und das ist der Sinn der Seite -
     * also trifft sie genau das gewaehlte.
     *
     * Ohne Angabe - "einfach so, kein Ziel" - wird nichts gebucht. Die
     * Spende ist trotzdem angekommen; sie zaehlt nur in keinen Balken,
     * und genau das stand dem Spender auch auf der Seite.
     *
     ...
     */
    public static function applyDonation(App $app, float $betrag, ?int $zielId = null): bool
    {
        if ($betrag <= 0 || $zielId === null) {
            return false;
        }
        ...
        $getroffen = $app->db->run(
            'UPDATE ' . self::TABLE . '
              ...
              WHERE id = :id',
            ['betrag' => self::money($betrag), 'id' => (string) $zielId]
        )->rowCount();
        ...
        if ($getroffen === 0) {
            ...
            // Spender.
            $app->log(self::SLUG . ': Spende ueber ' . self::money($betrag)
                . ' kam an, aber Ziel ' . $zielId . ' gibt es nicht mehr.');
