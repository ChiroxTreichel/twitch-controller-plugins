...
<div class="row">
    ...
    <a class="btn btn-ghost" href="<?= $e($url('/display/goals/tips')) ?>">
        <?= $e(translate('sl_tip.to_goals')) ?>
    </a>
</div>

<h1><?= $e(translate('sl_tip.appearance')) ?></h1>
<p class="lead"><?= $e(translate('sl_tip.appearance_lead')) ?></p>
...
<?php if ($missing !== []): ?>
    <div class="note note-error">
        <strong><?= $e(translate('sl_tip.missing_hint')) ?></strong>
...
<div class="card">
    <h2><?= $e(translate('sl_tip.required')) ?></h2>
    <p class="hint"><?= $e(translate('sl_tip.required_hint')) ?></p>
    ...
    <p class="hint"><?= $e(translate('sl_tip.format_hint')) ?></p>
</div>

<div class="card">
    <h2><?= $e(translate('sl_tip.markup')) ?></h2>

    <?php if (!$custom): ?>
        <div class="note note-warn"><?= $e(translate('sl_tip.is_default')) ?></div>
    <?php endif ?>
    ...
    */ ?>
    <form id="tip-appearance" method="post" action="<?= $e($ziel) ?>">
        ...
        <label class="field">
            <span class="hint"><?= $e(translate('sl_tip.field.html')) ?></span>
            <textarea class="input tip-code" name="html" rows="16" spellcheck="false"
                      <?= $canEdit ? '' : 'disabled' ?>><?= $e($html) ?></textarea>
        </label>

        <label class="field">
            <span class="hint"><?= $e(translate('sl_tip.field.css')) ?></span>
            <textarea class="input tip-code" name="css" rows="18" spellcheck="false"
                      <?= $canEdit ? '' : 'disabled' ?>><?= $e($css) ?></textarea>
        </label>

        <p class="hint"><?= $e(translate('sl_tip.no_script_hint')) ?></p>
        <p class="hint"><?= $e(translate('sl_tip.scope_hint')) ?></p>
    </form>
    ...
    <?php if ($canEdit): ?>
        <div class="row">
            ...
            <?= $view->render('_confirm', [
                'label'    => translate('sl_tip.reset_label'),
                'question' => translate('sl_tip.reset_question'),
                'confirm'  => translate('sl_tip.reset_label'),
                'action'   => $ziel,
